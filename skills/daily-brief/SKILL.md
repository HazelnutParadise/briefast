...
`market_outlook.direction` 只用 `up`（偏多）、`down`（偏空）、`range`（震盪、沒有明顯單向優勢）、`uncertain`（資料不足或重大訊號相互抵消，無法可靠判斷）。`summary_md` 用 1–3 句先講方向理由，再點出重要的反向訊號或限制；只能引用本次已查證、報告或相關會前 brief 已涵蓋的資訊。缺少某一類資料時不虛構，必要時以已公布資訊不足說明不確定性；不要把來源故障、蒐集批次或內部流程寫進報告。不得編造指數點位、漲跌幅、機率或盤中結果。
...
- `market_outlook` 必填，`direction` 只用 `up`、`down`、`range`、`uncertain`，`summary_md` 不得為空白；判斷目標是當日加權指數收盤相對前一交易日收盤。
