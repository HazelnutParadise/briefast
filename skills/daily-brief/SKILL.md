...
清單一律從下列指定端點取得，不從入口網站首頁自行找新聞——首頁是編輯檯排版，會漏掉窗口內的中小新聞。每個來源都是正式來源，不是補充。

**鉅亨網（主來源）** — 六個分類的列表 API，`{分類}` 代入下表值：
...
| 來源 | 清單端點 | 內文取得 | 重點 |
|---|---|---|---|
| 證交所 (TWSE) | `https://openapi.twse.com.tw/v1/opendata/t187ap04_L`（重大訊息）、`https://openapi.twse.com.tw/v1/opendata/t187ap05_L`（月營收） | JSON 即資料 | 整包拉回後依時間窗口過濾；請遵守下方節流規則 |
| 工商時報 (CTEE) | `https://www.ctee.com.tw/rss_web/livenews/{分類}`，分類用 `policy`（要聞）、`stock`（證券）、`finance`（金融）、`industry`（產業）、`house`（房市）、`tech`（科技） | 從 RSS 取得文章網址後以一般 HTTP 抓取 | 產業與個股深度報導。分類頁 HTML 會回 403，RSS 不會，不需要無頭瀏覽器 |
...
TWSE 對高頻請求會鎖 IP，抓取時遵守：

- 請求一律**序列執行、間隔至少 5 秒**，不得並行送出，也不得連續重試。
- 遇 `429` 或連線被拒，**等 60 秒重試一次**。
- 重試仍失敗就停止再打，依下方把關規則當作次要來源失敗處理（照常發布，並在執行回報列出缺 TWSE）。

### 來源成敗與完整性把關

逐一記錄每個來源這次是成功還是失敗。任何來源失敗就重試一次（TWSE 依上方節流規則等 60 秒再重試），再依結果分流：
