...
每個 `industries[].events[]` 都以獨立的 `headline` 與 `summary_md` 表達一組同主題動態。事件內文不複述標題，先指出新聞反映的供需或價格變化，再解釋影響落在產業鏈哪一段、哪些公司類型受益或承壓。

例如，事件標題是「記憶體合約價續漲，上游產品組合改善但模組廠毛利承壓」時，內文應寫：「原廠減產使通路庫存回到健康水位，合約價續漲有利上游製造商改善產品組合，但下游模組廠若無法同步轉嫁成本，毛利率可能承壓。」
