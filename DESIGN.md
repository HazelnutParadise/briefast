...
檔案紙色（用於 `/history/` 區全部頁面：歷史列表、任何日期的報告檢視、找不到報告頁，標示「所在區域」而非內容新舊）：暖調淡灰，淺色 paper 改 `#E4E0D9`、link 同步加深為 `#0B6870`（同色相，維持對比 ≥4.5:1）；深色 paper 改 `#282623`（其餘 token 不變）。取 greige 不取純冷灰，避免與暖紙面相衝；仍是中性紙色變化，不是新色相；首頁一律不得套用。「並非最新內容」提示帶另依日期判斷，只出現在早於最新報告的檢視。
...
- **大盤走勢預測（market-outlook）**：盤前總覽的頭條與完整內文先顯示，再於同一主欄接續顯示預測；標示 12.5px、方向襯線 24px、「預期走法」襯線 16.5px、「判斷依據」襯線 15px；有可靠三階段情境時，在方向後顯示定性 SVG 折線與開盤、盤中、尾盤相對前收的位置，標明「情境示意・相對前收，非點位或漲跌幅」，再呈現走法及理由。線條用中性文字色，收盤點偏多紅、偏空綠、震盪中性。與總覽內文以留白分隔，不加卡片或裝飾規線。舊報告缺圖或走勢文字時只省略對應部分，缺整個預測時整段省略。
...
- **多空標籤（tag）**：12.5px 粗體＋tint 底色；中性版無底色、1px rule 描邊、灰字。法說會預測沿用同一組：會後看漲紅、會後看跌綠、無法判斷中性；三種標籤固定寬 90px、高 28px，內容置中。
- **法說會卡片（conf-card）**：沿用多空欄的四直欄規線與斷點；欄頭是 3px 粗規線＋日期時間小字，其下股名襯線 18px、預測標籤、一句話標題、「會前報告 →」連結。區塊標題列右側以粗體小字顯示「近 N 場命中 X 場」，卡片下方以留白分隔的列列出最近結算：股名、日期、標籤、漲跌百分比（正紅負綠、持平灰）、命中／落空。
- **結果帶（result-band）**：已結算的會前報告頁在 dateline 下沿用歷史提示帶的深色帶，左側「已結算」標籤，中段會前與會後收盤及日期，右側漲跌百分比與命中／落空；文字一律帶色 `#F2EDE6`。
- **基本面表（fund）**：兩欄數字表，髮絲線分列，右對齊等寬數字，比較列在第三欄放「較上月 +2.70%」這類帶色字；季損益子表以 3px 粗規線開頭。
- **歷史列表**：日期粗體＋headline 灰字，髮絲線分隔；分頁當前頁 ink 底 paper 字。
- **頁尾深帶**：band 底、免責聲明 13.5px，永遠存在。
- **廣告版位（ad）**：報告頁唯一的廣告區塊，夾在個股多空判斷與產業新聞摘要之間——上方是當日結論，下方是佐證細節，是全頁唯一的閱讀斷點。上緣髮絲線＋12.5px 粗體字距 .14em 的「廣告」標示（ink-soft），內容區下緣再一道髮絲線收邊。廣告由第三方渲染，其圓角、陰影與配色不受本文件約束，規線與標示的作用就是把它界定成獨立區塊，不讓它冒充內文。沒有廣告可投放時整個版位收起（`:has(.ad-mount:empty)`），不留一個標著「廣告」的空框。
...
- ❌ 不加行銷 CTA、註冊引導——這是刊物，不是 landing page。廣告版位不在此列：它是營收來源而非本站的行銷引導，但必須維持被規線框住、標示清楚、且只有一個的狀態，不得散佈到其他區塊或偽裝成內文。
