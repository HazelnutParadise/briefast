...
檔案紙色（用於 `/history/` 區全部頁面：歷史列表、任何日期的報告檢視、找不到報告頁，標示「所在區域」而非內容新舊）：淺色 paper 改 `#EAE0CA`、link 同步加深為 `#0B6870`（同色相，維持對比 ≥4.5:1）；深色 paper 改 `#2A251E`（其餘 token 不變）。舊紙色是中性紙色的深淺變化，不是新色相；首頁一律不得套用。「並非最新內容」提示帶另依日期判斷，只出現在早於最新報告的檢視。
